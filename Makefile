...
.bin-deps:
	...
	GOBIN=$(BIN_DIR) go install github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-gateway@v2.11.2
	GOBIN=$(BIN_DIR) go install github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2@v2.11.2
	GOBIN=$(BIN_DIR) go install google.golang.org/protobuf/cmd/protoc-gen-go@v1.28.1
	GOBIN=$(BIN_DIR) go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@v1.2.0
	GOBIN=$(BIN_DIR) go install github.com/planetscale/vtprotobuf/cmd/protoc-gen-go-vtproto@v0.3.0
	GOBIN=$(BIN_DIR) go install github.com/mitchellh/gox@v1.0.1
	GOBIN=$(BIN_DIR) go install golang.org/x/tools/cmd/goimports@v0.1.9
...
.PHONY: build
build:
	go build -o ./bin/crnt-user-service ./cmd/crnt-user-service/main.go
...
.PHONY: .run
.run:
	go run ./cmd/crnt-user-service/main.go
